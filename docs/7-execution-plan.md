...
**완료 조건**
- [x] `npm run dev` 실행 시 Vite 개발 서버가 정상 구동된다
- [x] `npm run build` 실행 시 오류 없이 빌드된다
- [x] ESLint/Prettier 기본 규칙이 통과된다
- [x] `.env`가 `.gitignore`에 포함되어 있다
- [x] Tailwind 유틸리티 클래스가 빌드 출력에 포함된다
...
**완료 조건**
- [x] `QueryClientProvider`가 앱 루트에서 정상 래핑된다
- [x] 비인증 상태에서 `/` 접근 시 `/login`으로 리다이렉트된다
- [x] API 호출 시 `Authorization` 헤더가 자동으로 포함된다
- [x] 401 응답 수신 시 로그인 화면으로 리다이렉트된다
...
**완료 조건**
- [x] `setAuth` 호출 후 `localStorage`에 토큰이 저장된다
- [x] `clearAuth` 호출 후 인증 정보가 제거된다
- [x] 페이지 새로고침 후 토큰이 복원된다
- [x] `isAuthenticated`가 token 존재 여부와 일치한다
...
**완료 조건**
- [x] `register` 함수가 중복 이메일 시 Error를 throw한다
- [x] `login` 함수가 성공 시 `{ token, user }` 객체를 반환한다
- [x] `getMe` 함수가 JWT 헤더 포함 요청을 전송한다
...
**완료 조건**
- [x] 로그인 성공 시 authStore에 token이 저장되고 TodoPage로 이동한다
- [x] 로그인 실패 시 error 객체가 반환된다
- [x] 로그아웃 시 authStore가 초기화되고 QueryClient 캐시가 제거된다
...
**완료 조건**
- [x] 이메일/비밀번호 입력 후 로그인 버튼 클릭 시 API가 호출된다
- [x] 이메일 형식 오류 시 클라이언트 오류 메시지가 표시된다
- [x] 서버 오류(401) 시 안내 메시지가 표시된다
- [x] `isLoading` 중 버튼이 비활성화된다
- [x] 모바일/데스크톱 레이아웃이 정상 렌더링된다
...
**완료 조건**
- [x] 비밀번호 8자 미만 입력 시 오류 메시지가 표시된다
- [x] 영문/숫자 미혼합 시 형식 안내 메시지가 표시된다
- [x] 중복 이메일 시 서버 오류 메시지가 표시된다
- [x] 가입 성공 시 `/login`으로 이동한다
- [x] 이미 인증된 상태에서 접근 시 `/`로 리다이렉트된다
...
**완료 조건**
- [x] 각 함수가 올바른 HTTP 메서드와 경로로 요청을 전송한다
- [x] 오류 응답 시 Error를 throw한다
...
**완료 조건**
- [x] `useCategories` 반환 데이터가 카테고리 배열이다
- [x] 생성/수정/삭제 성공 시 목록이 자동으로 갱신된다
- [x] `isLoading`, `isError` 상태가 올바르게 반환된다
...
**완료 조건**
- [x] 카테고리 목록이 정상 렌더링된다
- [x] 이름 20자 초과 입력 시 오류 메시지가 표시되고 저장되지 않는다
- [x] 삭제 버튼 클릭 시 확인 다이얼로그가 표시된다
- [x] 다이얼로그에서 "취소" 클릭 시 삭제가 중단된다
- [x] 목록이 빈 경우 빈 상태 메시지가 표시된다
...
**완료 조건**
- [x] `getTodos`가 필터 파라미터를 쿼리스트링으로 올바르게 전달한다
- [x] 각 함수가 올바른 HTTP 메서드와 경로로 요청을 전송한다
...
**완료 조건**
- [x] 필터 파라미터 변경 시 쿼리가 자동으로 재실행된다
- [x] 생성/수정/삭제/완료 처리 후 목록이 자동 갱신된다
- [x] `isPending` 상태가 뮤테이션 중 올바르게 반환된다
...
**완료 조건**
- [x] `getTodoStatus`가 `completed`, `overdue`, `pending` 3가지 상태를 올바르게 반환한다
- [x] 종료일이 없는 할일은 `overdue`가 아닌 `pending`으로 판별된다
- [x] 완료된 할일은 종료일이 과거여도 `completed`로 반환된다
...
- [x] 상태 필터 선택 시 해당 상태 할일만 표시된다
- [x] "전체" 선택 시 필터가 초기화된다
- [x] 선택된 필터가 시각적으로 구분된다
- [x] 모바일에서 필터 탭이 가로 스크롤된다
...
**완료 조건**
- [x] 할일 목록이 정상 렌더링된다
- [x] 체크박스 클릭 시 완료/미완료 상태가 즉시 반영된다
- [x] `overdue` 상태 배지가 기한 초과 할일에 표시된다
- [x] 완료된 할일 제목에 취소선이 적용된다
- [x] 목록이 비었을 때 빈 상태 메시지가 표시된다
- [x] 로딩 중 스피너 또는 스켈레톤이 표시된다
...
**완료 조건**
- [x] 제목 미입력 시 "제목은 필수 항목입니다" 오류 메시지가 표시된다
- [x] 제목 100자 초과 시 오류 메시지가 표시된다
- [x] 설명 500자 초과 시 오류 메시지가 표시된다
- [x] 카테고리 드롭다운에 현재 카테고리 목록이 표시된다
- [x] 수정 모드에서 기존 값이 폼에 채워진다
...
**완료 조건**
- [x] 삭제 버튼 클릭 시 확인 다이얼로그가 표시된다
- [x] "취소" 클릭 시 다이얼로그가 닫히고 할일이 유지된다
- [x] "삭제" 클릭 시 할일이 영구 삭제되고 목록에서 제거된다
- [x] 삭제 처리 중 버튼이 비활성화된다
...
**완료 조건**
- [x] TodoPage에서 필터, 목록, 생성 버튼이 모두 렌더링된다
- [x] 헤더에 사용자 이메일이 표시된다
- [x] "로그아웃" 버튼이 동작한다
- [x] "할일 추가" 버튼 클릭 시 생성 폼 모달이 열린다
- [x] 모바일/데스크톱 반응형 레이아웃이 정상 동작한다
